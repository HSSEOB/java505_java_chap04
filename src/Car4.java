public class Car4 {
    ...
    //3. 생성자는 반환타입이 없다.
    //4. 생성자는 실행되면서 클래스의 멤버 변수를 초기화 하는데 사용함
    //5. 생성자끼리는 서로 호출하는 것이 가능하다.
    //6. 클래스의 멤버 메서드가 생성자를 호출 할 수 없음
    //7. 매개변수가 없는 형태의 생성자를 기본 생성자라고 한다.
    //8. 모든 클래스는 1개 이상의 생성자를 가지고 있다
    //9. 클래스에 생성자를 생략하면 컴파일러가 내용이 비어있는 기본 생성자를 자동 생성한다.
    //10. 클래스에 사용자가 생성자를 하나라도 선언 시 기본 생성자는 자동 생성되지 않음
    //11. 동일한 이름의 생성자를 여러개 선언하는 것을 생성자 오버로딩이라고 하며, 매개변수의 개수, 타입, 순서가 다를 경우 사용 할 수 있다.
